...
internal class DuckLakeConnectorMetadata(
    ...
    override fun getTableSchema(
        ...
        val handle = asDuckLakeHandle(tableHandle)
    ...
    private companion object {
        private fun asDuckLakeHandle(handle: ConnectorTableHandle): DuckLakeTableHandle {
            if (handle is DuckLakeTableHandle) {
                return handle
            }
            throw IllegalArgumentException(
                "Expected DuckLakeTableHandle, got ${handle.javaClass.name}",
            )
        }
    }
}
