...
- [x] `DuckLakePredicateConverter` — maps `ConnectorExpression` conjuncts → `ColumnRangePredicate`s: `col <op> literal` comparisons, `BETWEEN`, and **membership** (`col IN (…)`, same-column `col = a OR col = b …`) collapsed to the typed `[min..max]` span of their values (shared recognizer `DuckLakeMembership`). Skipped: functions / LIKE / IS NULL / NE / NOT IN / mixed-column or range `OR` — **no function pushdown by design**.
