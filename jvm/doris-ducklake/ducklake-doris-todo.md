...
- [ ] **BUCKET partition pruning** — the one genuine pruning gap. Source-column stats can't prune buckets (each bucket file holds a scattered value range), so it needs *transform-aware* matching: compute `bucket(N, predicate_literal)` and keep only that bucket's files (via `getFilePartitionValues`). ⚠️ **Do NOT hand-roll the hash.** trino-ducklake's `DucklakeBucketPartitionMatcher` / `DucklakePartitionComputer.computeBucket` reuses *"the exact hash function used at write time"* via Trino's block encoding — a mismatched hash silently drops matching rows. The safe path: extract an engine-agnostic bucket-hash into `ducklake-catalog` and call it from BOTH plugins so they're provably identical. (Touches the shared catalog lib — scope deliberately.)
