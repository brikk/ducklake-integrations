 ...
 *
 * Narrows an opaque Doris SPI `ConnectorTableHandle` (from the metadata and
 * scan-plan SPI) to a concrete plugin handle type [T], throwing
 * [IllegalArgumentException] naming the actual runtime class when it is the
 * wrong type (or null).
 *
 ...
 * `javaClass.name` ceremony at all call sites. Reified so call sites read
 * `handle.asDuckLakeHandle()` with the target type inferred.
 *
