...
| Date truncate | `date_trunc(unit, x) -> [same]` | `date_trunc(part, x)` | ⚠️ Units: Trino accepts `second, minute, hour, day, week, month, quarter, year`; DuckDB adds `microseconds, milliseconds, decade, century, millennium`. Push when unit is in the intersection. Both ISO-week-aligned for `week`. |
...
| Date diff | `date_diff(unit, t1, t2) -> bigint` | `date_diff(part, t1, t2)` | ⚠️ Semantics: both return **integer count of unit-boundaries crossed**, not whole-units elapsed. e.g. `date_diff('month', '2024-01-31', '2024-02-01') = 1` in both. Push when unit name is shared; verify `week`. |
