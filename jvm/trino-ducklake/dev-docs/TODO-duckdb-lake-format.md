...
- **2026-05-05** — Arrow-stream writer landed (alternative to Appender). New session property `ducklake.duckdb_writer_mode` chooses `appender` (default) or `arrow_stream` for the duckdb-format write path. `arrow_stream` mode runs `INSERT INTO ducklake_out.main.t SELECT * FROM <registered_stream>` on a worker thread; the page-sink thread feeds Trino `Page`s through a small bounded queue (`ArrayBlockingQueue` cap 2); a custom `ArrowReader` subclass pulls one Page per `loadNextBatch()`, populates a `VectorSchemaRoot`, returns. `Data.exportArrayStream` wires the reader into a real `ArrowArrayStream` (the user pointed out — DuckDB's `registerArrowStream` strictly requires that exact class, so we use the C-data export path). End-of-stream signaled via a sentinel `Page` enqueued from `finishAndBuildFragment`. Memory bounded to a few Pages instead of buffering everything. Per-cell JNI of the Appender path replaced by one C-data round-trip per Page. New test class `TestDucklakeDuckDbArrowStreamWriter` (6 cases): scalars, NULLs, temporal+decimal, column stats populated, bulk insert through the queue. Both writers pass the same correctness assertions and produce the same shape of catalog output. Open question A in this doc closed in favor of "both modes coexist; benchmark to decide which is the right default."
