...
> **maintenance ops**, **catalog backends**, and **row lineage** is superseded — read those rows as
> historical. On change-feed semantics (datafusion is the closest reference impl of `table_changes`):
> we now read the embedded row-lineage column (parquet field-id 2147483540) like datafusion's
> `RowIdExec`, so lineage-preserving UPDATEs pair into `update_preimage`/`update_postimage`; and
> since 2026-07-06 (F7) the JVM connector also WRITES the lineage column on its own UPDATE/MERGE
> (`write_row_lineage` session property) — beyond datafusion, which reads but does not emit it on
> updates. Without the property a Trino-written UPDATE surfaces as `delete`+`insert`.
...
| Row lineage (`rowid` virtual column) | Opt-in via `DuckLakeCatalog::with_row_lineage(true)`; appends BIGINT `rowid` at end of schema. `RowIdExec` injects `row_id_start + position` per file; also detects the embedded `_ducklake_internal_row_id` column (Iceberg-reserved field-id `2_147_483_540`) that UPDATE / compaction writes. Catalog plumbing (`row_id_start`, `record_count`, `ducklake_table_stats(next_row_id)`) is wired in all 4 providers + SQLite & Postgres writers. Multicatalog reader projects the same fields (#124, 2026-05-29). | Tracked in `DESIGN-virtual-columns.md` as `$row_id` and listed in `TODO-READ-MODE.md` under "Add DuckDB-equivalent virtual columns" — **not implemented**. We expose `$row_id` (= `row_id_start + position`) + `$snapshot_id`/`$path`/`$file_row_number`/`$file_size_bytes` as hidden queryable virtual columns; the change feed reads the embedded `_ducklake_internal_row_id` (field-id 2147483540) for preserved lineage. | **JVM ahead as of 2026-07-06 (F7)** — we read the embedded lineage column (like Rust's `RowIdExec`) AND write it on our own UPDATE/MERGE (`write_row_lineage`), which datafusion does not. The queryable `$row_id` virtual is still positional (embedded lineage is applied only inside the change feed, not exposed as a scan column); wiring embedded lineage into the `$row_id` virtual too is the remaining nicety. |
