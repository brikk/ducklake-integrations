...
> `RowIdExec`, so lineage-preserving UPDATEs pair into `update_preimage`/`update_postimage`; Trino's
> own writes emit no lineage column, so a Trino-written UPDATE surfaces as `delete`+`insert`.
