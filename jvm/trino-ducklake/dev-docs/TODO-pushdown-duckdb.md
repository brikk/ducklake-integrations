...
- Step 4 (date / time) — **chunks 1 + 2 + 3 shipped (narrow chunk 3); chunk 3.5 queued**. Chunk 1 (round 6j) added the translator's argument-type-gate registry plus 12 new pushable entries (Tier A DATE-only, Tier B DATE or TIMESTAMP no-TZ). Chunk 2 added `TrinoTimeZoneNormaliser` and threaded Trino's session `TimeZoneKey` through the page-source / executor stack so `SET TimeZone` fires at attach in both `InProcessDuckDbExecutor` and `QuackDuckDbExecutor`. Chunk 3 added the `pushdown_timestamp_with_timezone` session property, threaded `ConnectorSession` through the translator's gates, and promoted ONLY `to_unixtime/1` to Tier C — the other zone-dependent extracts (`year/month/day/quarter/hour/minute/second/millisecond/date_trunc/date_diff`) stayed blocked over WTZ because `DucklakeArrowToPageConverter:380-388` hardcodes `UTC_KEY`, making Trino above-scan year() always UTC-aligned while DuckDB-side uses session zone. **Chunk 3.5 (queued)** fixes the converter to honour the Arrow schema TZ instead — empirically validated path, see [TODO-pushdown-datetime.md](TODO-pushdown-datetime.md). Plan in [PLAN-pushdown-datetime.md](PLAN-pushdown-datetime.md), empirical TZ findings in [REPORT-datetime-tz-handling.md](REPORT-datetime-tz-handling.md).
