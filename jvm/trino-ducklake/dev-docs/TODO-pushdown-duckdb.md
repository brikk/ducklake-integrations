...
- Step 4 (date / time) — **chunks 1 + 2 shipped**; chunk 3 pending. Chunk 1 (round 6j) added the translator's argument-type-gate registry (closes a latent bug where date macros would push for any arg type) plus 12 new pushable entries — Tier A (DATE-only) and Tier B (DATE or TIMESTAMP no-TZ). Chunk 2 added `TrinoTimeZoneNormaliser` to production, threaded Trino's session `TimeZoneKey` through `DucklakePageSourceProvider → DuckDbFilePageSource → ExecutionRequest`, and wired `SET TimeZone` at attach in both `InProcessDuckDbExecutor` and `QuackDuckDbExecutor` (server-side via `quack_query_by_name`). Closes the portability bomb (DuckDB used to inherit JVM system TZ → silent variation across CI / dev / prod); SET TimeZone now fires unconditionally for a deterministic baseline regardless of tier. Planned in [PLAN-pushdown-datetime.md](PLAN-pushdown-datetime.md), empirical TZ findings in [REPORT-datetime-tz-handling.md](REPORT-datetime-tz-handling.md), chunk tracker in [TODO-pushdown-datetime.md](TODO-pushdown-datetime.md).
