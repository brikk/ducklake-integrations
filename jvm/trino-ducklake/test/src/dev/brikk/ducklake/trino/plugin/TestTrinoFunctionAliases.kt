...
/**
 * SQL-level acceptance tests for the `trino_*` macro layer.
 *
 *
 * The fixtures in [semanticCases] are the "brain" of pushdown
 * semantics: one or more rows per `(trino_name, arg_count)` entry in
 * `trino_meta()`, with the expected DuckDB-evaluated result chosen to
 * match Trino's documented behavior. The [testMetaMatchesFixtures]
...
class TestTrinoFunctionAliases {
    ...
    fun testTrinoMacroSemantics() {
        openConnectionWithExtension().use { conn ->
            conn.createStatement().use { stmt ->
                for (c in semanticCases()) {
                    val actual = scalar(stmt, c.sql)
    ...
    @Test
    @Throws(Exception::class)
    fun testTrinoMetaCatalogMatchesMacros() {
        openConnectionWithExtension().use { conn ->
            conn.createStatement().use { stmt ->
                val meta = readMeta(stmt)
                val installed = installedTrinoMacros(stmt)
                for (entry in meta) {
    ...
    @Test
    @Throws(Exception::class)
    fun testMetaMatchesFixtures() {
        openConnectionWithExtension().use { conn ->
            conn.createStatement().use { stmt ->
                val meta = readMeta(stmt)
                val covered = HashSet<NameArity>()
                for (c in semanticCases()) {
    ...
    fun testJavaPushableSetMatchesDuckDbMeta() {
        ...
        openConnectionWithExtension().use { conn ->
            conn.createStatement().use { stmt ->
                ...
                val javaSet: Set<DuckDbExpressionTranslator.NameArity> =
    ...
    @Test
    @Throws(Exception::class)
    fun testUnicodeCaseFoldingMatchesTrinoSpec() {
    ...
    @JvmRecord
    data class SemanticCase(val label: String, val nameArity: NameArity, val sql: String, val expected: Any?) {
    ...
    companion object {
         ...
         * LOADed. Path comes from [TrinoParityExtensionResolver] — same code
         ...
         * `(cd duckdb-trino-parity-extension && GEN=ninja make)`).
         */
        ...
        @JvmStatic
        fun semanticCases(): List<SemanticCase> {
            return listOf(
                    // String — round 1
        ...
        private fun c(label: String, name: String, arity: Int, sql: String, expected: Any?): SemanticCase {
            return SemanticCase(label, NameArity(name, arity), sql, expected)
        }
        ...
        private fun readMeta(stmt: Statement): Set<NameArity> {
            ...
            stmt.executeQuery(
                    "SELECT trino_name, arg_count FROM trino_meta() ORDER BY trino_name, arg_count").use { rs ->
                while (rs.next()) {
        ...
        @Throws(Exception::class)
        private fun installedTrinoMacros(stmt: Statement): List<String> {
            val installed = ArrayList<String>()
            stmt.executeQuery(
                    "SELECT function_name FROM duckdb_functions() WHERE function_name LIKE 'trino\\_%' ESCAPE '\\'").use { rs ->
                while (rs.next()) {
