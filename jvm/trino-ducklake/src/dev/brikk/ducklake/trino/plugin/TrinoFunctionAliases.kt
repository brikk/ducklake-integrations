...
/**
 * Loader for the `trino_parity` DuckDB extension, which registers all the
 * `trino_<name>(...)` macros and the `trino_meta()` table macro
 * the connector's pushdown translator relies on.
 *
 * Historical note: this class used to read a classpath SQL resource and run
 * `CREATE OR REPLACE MACRO` statements per attach as a fallback for
 ...
 * [TrinoParityExtensionResolver]); operators can override with the
 * `ducklake.duckdb.parity-extension-path` catalog property.
 *
 * The class is two static helpers: one for the in-process JDBC executor
 * (LOAD on a local [Connection]) and one for the Quack executor
 * (LOAD over the wrapper, using the same [Statement] that runs
 * `drainWrappedQuery`). Both throw [SQLException] on failure so
...
object TrinoFunctionAliases
{
     ...
     * executor opened. Uses a scratch [Statement] so a failure
     ...
     * have been opened with `allow_unsigned_extensions=true` as a
     ...
     * caller's `init` statement is used to forward the LOAD through
     * `QuackDuckDbExecutor.drainWrappedQuery`. The server-side DuckDB
     * must have been started with `-unsigned` (the testcontainer
    ...
    /**
     * Functional handle to `QuackDuckDbExecutor.drainWrappedQuery` that
