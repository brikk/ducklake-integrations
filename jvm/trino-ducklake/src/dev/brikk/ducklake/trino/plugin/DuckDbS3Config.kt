 ...
 * (`s3.endpoint`, `s3.region`, `s3.aws-access-key`,
 * `s3.aws-secret-key`, `s3.path-style-access`) that Trino's
 * `FileSystemModule` consumes for the parquet path — the user only configures
 * S3 once. Reading these directly from the config map avoids binding the same
 * keys to two airlift `@Config` classes (which the Bootstrap would treat as
 * a duplicate-binding error).
 *
 * All fields are optional; when [endpoint] is absent the connector falls
 * back to AWS-default endpoint resolution. [useSsl] is derived from the
 * endpoint URL scheme (defaults to `true` when no endpoint is given).
 */
@JvmRecord
data class DuckDbS3Config(
        @get:JvmName("endpoint") val endpoint: String?,
        @get:JvmName("region") val region: String?,
        @get:JvmName("accessKey") val accessKey: String?,
        @get:JvmName("secretKey") val secretKey: String?,
        @get:JvmName("pathStyleAccess") val pathStyleAccess: Boolean,
        @get:JvmName("useSsl") val useSsl: Boolean) {
    /**
     * Render a DuckDB `CREATE SECRET IF NOT EXISTS ducklake_s3 (TYPE S3, ...)` statement
     * using these settings. `IF NOT EXISTS` (NOT `OR REPLACE`) is load-bearing on the
     * Quack execution-engine path, where the secret is server-instance-scoped and shared across
     * concurrent sessions:
     *
     *  - `OR REPLACE` under concurrency is a proven failure (live 2026-06-10, pinned by
     ...
     *  - `IF NOT EXISTS` on an existing secret performs no catalog write at all (probed:
    ...
    fun toObjectStoreEnv(): Map<String, String> {
        ...
        accessKey?.let { env["AWS_ACCESS_KEY_ID"] = it }
        secretKey?.let { env["AWS_SECRET_ACCESS_KEY"] = it }
        region?.let { env["AWS_REGION"] = it }
        endpoint?.let { e ->
